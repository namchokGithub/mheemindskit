    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-sm">
        <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-border bg-secondary/50 px-3 py-2">
          <Button type="button" size="sm" onClick={() => run()} disabled={!input.trim()}>{actionIcon}{actionLabel}</Button>
          ...
          <Button type="button" variant="outline" size="sm" onClick={() => download(output, `mindskit-output.${extension}`, extension === 'csv' ? 'text/csv;charset=utf-8' : 'text/yaml;charset=utf-8')} disabled={!hasConverted}><Download />Download {extension.toUpperCase()}</Button>
        </div>
        {error && <div className="shrink-0 px-3 pt-3"><ToolStatus state="invalid" message={error} /></div>}
        <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-2">
          <section className="flex min-h-0 min-w-0 flex-col border-b border-border lg:border-r lg:border-b-0"><span className="px-3 py-1.5 text-sm font-medium text-muted-foreground">JSON input</span><CodeEditor bare value={input} onChange={(value) => { setInput(value); setError(''); setHasConverted(false) }} placeholder="Paste JSON here…" wrap ariaLabel="JSON input" /></section>
          <section className="flex min-h-0 min-w-0 flex-col"><div className="flex items-center justify-between px-3 py-1.5"><span className="text-sm font-medium text-muted-foreground">{outputLabel}</span><CopyButton value={hasConverted ? output : ''} /></div><CodeEditor bare value={hasConverted ? output : ''} readOnly placeholder="Converted output will appear here." wrap ariaLabel={outputLabel} language="text" /></section>
        </div>
      </div>
    </div>
  )
